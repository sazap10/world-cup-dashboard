---
name: run-app
description: Launch and drive the World Cup 2026 dashboard (Vite SPA + single-process Go server in server/*.go). Use to run/start/preview the app, smoke-test the /api/wc/matches feed cache, or exercise live data without a real football-data.org token. Covers seed-only mode and a mock-upstream "live" mode, plus browser verification.
---

# Run the World Cup 2026 dashboard

The app is a Vite React SPA served (in prod) by a single Go process
(`server/*.go`), which also exposes the cached `/api/wc/matches` feed. Running
it = building that binary and driving it as a user would. (The Vite *dev* server
runs the same feed cache from `server/feed-cache.mjs` — that JS file stays.)

## 0. Build first (the server serves `dist/`)

```bash
npm run build          # tsc -b && vite build  → produces dist/
npm run build:server   # → bin/server (static Go binary)
```

The server reads the SPA from `DIST_DIR` (default `dist/`, relative to the
working dir). If `dist/` is missing it fails to start.
...
```bash
PORT=8080 ./bin/server                 # run in background
...
pkill -f 'bin/server'; pkill -f mock-upstream.mjs
```
