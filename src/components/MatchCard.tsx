...
export function MatchCard({ match, showRound = true }: Props) {
  ...
  const startsSoon = match.status === 'upcoming' && Date.parse(match.kickoff) - nowMs < 48 * 3600_000;
  ...
  return (
    <article className={`match-card match-card--${match.status}`}>
        ...
        <span className="match-card__status">
          ...
          {match.status === 'upcoming' && (
            <span className="status-upcoming">
              <span className="status-upcoming__day">{relativeDayLabel(match.kickoff, tz, nowMs)}</span>
      ...
      <div className="match-card__teams">
        <TeamRow side={{ code: match.home, goals: homeGoals, isWinner: homeWins, faded: awayWins }} />
        <TeamRow side={{ code: match.away, goals: awayGoals, isWinner: awayWins, faded: homeWins }} />
      </div>
